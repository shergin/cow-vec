...
Storage is append-only while anything else can see it. `set`, `pop`,
`remove`, and `clear` on a vector with live clones leave the old values
in place, because a clone might still be pointing at them, and the arenas
hand out pointers that cannot move.
...
// Or build a compacted copy and keep the original as it is:
let copy = vec.clone_compacted(0);
assert_eq!(copy, vec);
```

Drop every clone of a lineage and all of its arenas go with it. A vector
that outlives its clones takes their arenas back on its next mutation.
Keep-alive chains drop iteratively, so a version chain hundreds of
thousands of generations deep unwinds without blowing the stack.

## Limitations

- **`&mut T` costs a clone while the storage is shared.** `make_mut`
  hands out the slot directly only when nothing else references the
  storage; otherwise it clones into a fresh slot first. There is no
  in-place mutation that other clones could see — that is the point.
